...
import { ResourceListWithFilter } from "@/components/resources/ResourceListWithFilter";
  ...
  return (
    <div className="mx-auto w-full max-w-[1200px] px-6 py-10">
      <div className="mx-auto max-w-4xl">
        ...
        {/* Course header card */}
        <div className="mb-8 overflow-hidden rounded-2xl" style={{ border: "0.5px solid #e4e4f0" }}>
          {/* Colour banner */}
          <div className="relative px-8 py-7" style={{ background: gradient }}>
            <div style={{ position: "absolute", inset: 0, background: `radial-gradient(ellipse at 80% 50%, ${glow} 0%, transparent 65%)`, pointerEvents: "none" }} />
            <div className="relative z-10 flex flex-wrap items-center gap-2">
              <span className={`rounded-full px-3 py-1 font-mono text-[11px] font-semibold uppercase tracking-wider ${pill}`}>
                {course.course_code}
              </span>
              <span className="rounded-full bg-white/10 px-2.5 py-1 font-mono text-[10px] font-medium uppercase tracking-wider text-white/80">
                {course.semester} {course.year}
              </span>
            </div>
            <h1 className="relative z-10 mt-3 text-2xl font-bold text-white sm:text-3xl">
              {course.course_name}
            </h1>
            <p className="relative z-10 mt-1 flex items-center gap-1.5 text-sm text-white/70">
          ...
          {/* White body */}
          <div className="flex flex-col gap-4 bg-white px-8 py-5 sm:flex-row sm:items-center sm:justify-between">
            <div className="flex flex-col gap-3">
              {course.description && (
                <p className="max-w-xl text-sm leading-relaxed text-zinc-600">{course.description}</p>
              )}
            ...
            <div className="flex shrink-0 items-center gap-2">
              <BookmarkButton courseId={courseId} courseName={course.course_name} />
              <CopyLinkButton />
              {user && (
        ...
        {/* Resources with type filter + download-all */}
        ...
        {!user && (
          <p className="mt-8 text-center text-sm text-zinc-500">
            <Link href={`/login?redirectTo=/courses/${courseId}/upload`} className="font-semibold text-zinc-900 hover:underline">
              Sign in
            </Link>{" "}
